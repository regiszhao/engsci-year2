...
_start:
	...
	LDR		R0, =50000000	    // countdown = 50M
	MOV		R3, #0		        // Direction Tracker (0 = IN, 1 = OUT)
	STR		R0, [R8]			// stores value of R0 into R8
    MOV		R0, #0b011		// A = 1 (auto-reload), E = 1 (enable), I = 0 (interrupt)
    STR		R0, [R8, #8]		
...
DISPLAY:
	STR	R6, [R1]				// Display LEDs, store value of R6 into R1
...
ROT_IN:
	AND 	R12, R6, #0b10000	
	CMP		R12, #0b10000	// Check to see if left side is on inside boundary
