...
% Construction of inline function
f = @(t,y) y .* tan(t) + sin(t);
% Initial conditions
t0 = 0;
y0 = -1/2;
% Time we will integrate until
t1 = pi;
soln = ode45(f, [t0, t1], y0);
...
% The error is largest at |x = 1.55881|. This is because the slope
% of the solution is the largest there - given a certain step size, the
% larger the slope, the less accurate numerical methods of integration will
% be.
...
% Compute the exact solution; y = (3t-2)^(1/3)
...
% Plotting both solutions
plot(soln.x, soln.y, soln.x, yexact, 'LineWidth', 2);
xlabel('t');
ylabel('y');
...
% Larger slopes require smaller step sizes in order to maintain accuracy
% during integration. The solution's slope becomes extremely large before
% reaching t = 1, so large that MATLAB's smallest step size value allowed
% is unable to meet its integration tolerances, hence the error message.
